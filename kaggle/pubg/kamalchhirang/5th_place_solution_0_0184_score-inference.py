# Source : https://www.kaggle.com/kamalchhirang/5th-place-solution-0-0184-score/
...
def featureModify(isTrain):
    if isTrain:
        all_data = pd.read_csv(os.path.join(dataDirPath, "train_V2.csv")) #'../input/train_V2.csv') 
        all_data = all_data[all_data['maxPlace'] > 1]
        all_data = reduce_mem_usage(all_data)
        all_data = all_data[all_data['winPlacePerc'].notnull()]
    else:
        all_data = pd.read_csv(os.path.join(dataDirPath, "test_V2.csv"))#'../input/test_V2.csv')
...
def ReadTrainingData():    
    global X_train,features_label
    X_train,features_label = featureModify(True)
    # return X_train, features_label
...
model = None
def Train():
    global X_train,features_label, model
    ...
    joblib.dump(model, os.path.join(scriptDirPath, "LightGBMModel.pkl"))
    del featureImp,features_label
    gc.collect()

def ReadTestData():
    X_test,features_label = featureModify(False) 
    return X_test, features_label

def RunInference():                  
    global model
    model = joblib.load(os.path.join(scriptDirPath, "LightGBMModel.pkl"))
    X_test,features_label = ReadTestData()     
    ...
    df_test = pd.read_csv(os.path.join(dataDirPath, 'test_V2.csv'))#'../input/test_V2.csv')
...
def RunTrainingAndInference():
    ...
    if runTraining==True:
        Train()
    input("Done training model ... press any key to start inference")
    if profileInference:
