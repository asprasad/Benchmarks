...
def ExampleTestDataExploration():
...
def TrainModels():
    ...
    **params
    ...
    return (rfe, model, user_answers_df, content_answers_df)

#[Ashwin] : The following (commented out) code seems to be some kind of hyper parameter optimization. I think
# it is just exploring the space of some hyper parameters to find a good model. The above code
# has hard-coded hyper-parameter values that were probably determined by running the code below. 
...
def RunInference(rfe, model, user_answers_df, content_answers_df):
    ...
    for start in range(0, numberOfRows, 100): #(test_df, sample_prediction_df) in iter_test:
