...
def DoTrainingAndInference():
    ...
    for j in range(num_vars):
        ...
        # MODEL WITHOUT MAGIC
        ModelWithoutMagic(j, train2, param, evals_result, all_oofB, all_predsB)
...
def ModelWithoutMagic(j, train2, param, evals_result, all_oofB, all_predsB):
    # MODEL WITHOUT MAGIC
    features = ['var_'+str(j)]
    ...
    if plotGraphs == True:
        ...
        mn = min(train2[train2['target']==0]['var_'+str(j)].min(), train2[train2['target']==1]['var_'+str(j)].min())
        mx = max(train2[train2['target']==0]['var_'+str(j)].max(), train2[train2['target']==1]['var_'+str(j)].max())
